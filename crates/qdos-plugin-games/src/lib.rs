...
// Allow pre-existing warnings from before plugin extraction
#![allow(dead_code)]
...
use qdos_sound::{ChiptuneMelody, ChiptuneMusic};
...
impl GamesPlugin {
    pub fn new() -> Self {
        ...
        let sounds_enabled = config::GamesConfig::load()
            .map(|c| c.play_sounds)
            .unwrap_or(true);
    ...
    pub fn load_from_config(&mut self) {
        if let Ok(cfg) = config::GamesConfig::load() {
            // Decode leaderboards from base64 JSON
            self.state.load_leaderboards(cfg.get_leaderboards());

            // Decode clicker state from base64 JSON
            self.state.clicker = cfg.get_clicker_state();

            // Decode player stats from base64 JSON
            self.stats = StatsTracker::new(cfg.get_player_stats());

            // Decode achievements from base64 JSON
            self.achievements = AchievementManager::from_state(cfg.get_achievements());
        }
    }

    /// Save persisted data to config file
    pub fn save_to_config(&self) {
        if let Ok(mut cfg) = config::GamesConfig::load() {
            // Encode leaderboards to base64 JSON
            cfg.set_leaderboards(&self.state.leaderboards);

            // Encode clicker state to base64 JSON
            cfg.set_clicker_state(&self.state.clicker);

            // Encode player stats to base64 JSON
            cfg.set_player_stats(&self.stats.stats);

            // Encode achievements to base64 JSON
            cfg.set_achievements(&self.achievements.state);
    ...
    pub fn open_modal(&mut self) {
        ...
        self.sounds_enabled = config::GamesConfig::load()
            .map(|c| c.play_sounds)
            .unwrap_or(true);
