//! Git Plugin for QDOS
//!
//! Provides Git integration as a standalone plugin crate.
...
impl Plugin for GitPlugin {
    ...
    fn status_info(&self, cwd: &PathBuf) -> Option<PluginStatusInfo> {
        ...
        if self.ahead > 0 || self.behind > 0 {
            parts.push(format!("^{}v{}", self.ahead, self.behind));
        }
        if self.staged > 0 {
    ...
    fn draw_modal(&self, frame: &mut Frame, area: Rect, colors: &ThemeColors) {
        if let Some(ref state) = self.modal_state {
            modal::draw_git_modal(frame, area, state, colors);
        }
    }
    ...
    fn help_content(&self) -> Vec<String> {
        vec![
            ...
            "".to_string(),
            "Status bar shows: branch name, ^ahead/vbehind counts,".to_string(),
            ...
            "  Up/Dn     Navigate lists".to_string(),
...
// Self-registration with inventory
inventory::submit! {
    PluginRegistration::new("git", || Box::new(GitPlugin::new()))
}
...
mod tests {
    ...
    fn test_menu_item() {
        let plugin = GitPlugin::new();
        let item = plugin.menu_item().unwrap();
