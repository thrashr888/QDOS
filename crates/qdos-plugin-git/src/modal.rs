...
use crate::state::{GitMenuItem, GitState, GitView, RemoteAction, SubmoduleStatus};
...
/// Draw Git modal
pub fn draw_git_modal(frame: &mut Frame, area: Rect, state: &GitState, colors: &ThemeColors) {
            ...
            GitView::Commit => {
                let mut lines = vec![
                    ...
                    Line::from(vec![
                        ...
                        Span::styled("_", Style::default().fg(colors.yellow()).bg(colors.red())),
                    ]),
                ];
                ...
                if state.submodules.is_empty() {
                    ...
                    for (i, submodule) in state.submodules.iter().enumerate() {
                        ...
                        let status_color = match submodule.status {
                            SubmoduleStatus::Initialized => colors.green(),
                            SubmoduleStatus::Uninitialized => colors.grey(),
                            SubmoduleStatus::Modified => colors.yellow(),
                            SubmoduleStatus::Conflict => colors.red(),
                            SubmoduleStatus::OutOfDate => colors.blue(),
                        };
            ...
            GitView::Worktrees => {
                ...
                if state.worktrees.is_empty() {
                    ...
                    for (i, worktree) in state.worktrees.iter().enumerate() {
                        ...
                        let lock_indicator = if worktree.is_locked { "[L]" } else { "   " };
    ...
    let help_hints: Vec<(&str, &str)> = if !state.is_repo {
        ...
        match state.view {
            ...
            GitView::Diff => vec![("Up/Dn", "scroll"), ("PgUp/Dn", "fast"), ("ESC", "back")],
            GitView::Commit => vec![
                ("type", "message"),
                ("Shift+Enter", "newline"),
                ("Enter", "commit"),
            ...
            GitView::Remote => vec![("Up/Dn", "select"), ("Enter", "execute"), ("ESC", "back")],
            GitView::Config => vec![
                ("Up/Dn", "scroll"),
                ("PgUp/Dn", "fast"),
                ("R", "refresh"),
