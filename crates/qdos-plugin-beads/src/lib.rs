...
impl Plugin for BeadsPlugin {
    ...
    fn draw_modal(&self, frame: &mut Frame, area: Rect, colors: &ThemeColors) {
        if let Some(ref state) = self.modal_state {
            modal::draw_beads_modal(frame, area, state, colors);
        }
    }
...
inventory::submit! {
    PluginRegistration::new("beads", || Box::new(BeadsPlugin::new()))
}
...
mod tests {
    ...
    #[test]
    fn test_beads_plugin_new() {
        let plugin = BeadsPlugin::new();
        assert_eq!(plugin.id(), "beads");
        assert_eq!(plugin.name(), "Beads Issue Tracker");
        assert!(plugin.capabilities().has_modal);
    }

    #[test]
    fn test_beads_plugin_default() {
        let plugin = BeadsPlugin::default();
        assert_eq!(plugin.id(), "beads");
    }
}
