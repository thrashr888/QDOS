...
use crate::{GitMenuItem, GitState, GitView, RemoteAction};
                ...
                if state.submodules.is_empty() {
                    ...
                    for (i, submodule) in state.submodules.iter().enumerate() {
                        ...
                        let status_indicator = match submodule.status {
                            crate::SubmoduleStatus::Initialized => "+",
                            crate::SubmoduleStatus::Uninitialized => "-",
                            crate::SubmoduleStatus::Modified => "*",
                            crate::SubmoduleStatus::Conflict => "!",
                            crate::SubmoduleStatus::OutOfDate => "^",
                        };

                        let status_color = match submodule.status {
                            crate::SubmoduleStatus::Initialized => colors.green(),
                            crate::SubmoduleStatus::Uninitialized => colors.grey(),
                            crate::SubmoduleStatus::Modified => colors.yellow(),
                            crate::SubmoduleStatus::Conflict => colors.red(),
                            crate::SubmoduleStatus::OutOfDate => colors.blue(),
                        };
