...
use crate::api::{embeddings::create_embeddings_provider, AIApiConfig, ApiError};
...
impl SemanticSearch {
    ...
    pub fn stats(&self) -> &crate::indexer::IndexStats {
        self.indexer.stats()
    }
    ...
    pub fn prepare_file_for_batch(
        &self,
        path: &std::path::Path,
    ) -> Option<(String, crate::vector::EntryMetadata, u64)> {
    ...
    pub fn index_batch(
        &mut self,
        files: Vec<(std::path::PathBuf, String, crate::vector::EntryMetadata)>,
...
mod tests {
    ...
    fn test_semantic_search_creation() {
        ...
        use crate::indexer::{FileIndexer, IndexConfig};
