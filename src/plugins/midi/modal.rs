//! MIDI Player plugin modal rendering
//!
//! UI for the MIDI player plugin.

use super::state::{MidiPlayer, MidiState, MidiView};
...
pub fn draw_midi_modal(frame: &mut Frame, area: Rect, state: &MidiState, colors: &ThemeColors) {
    match state.view {
        MidiView::Menu => draw_menu(frame, area, state, colors),
        MidiView::Playing => draw_playing(frame, area, state, colors),
        MidiView::Error => draw_error(frame, area, state, colors),
    }
}

fn draw_menu(frame: &mut Frame, area: Rect, state: &MidiState, colors: &ThemeColors) {
    let modal_area = centered_modal_area(area);
    let modal = ModalFrame::themed(modal_area, " MIDI Player ", colors);
    ...
    let file_info = if let Some(path) = &state.file_path {
        ...
        vec![Line::from(Span::styled(
            "No file selected - select a .mid file",
            Style::default().fg(colors.grey()),
        ))]
    };
...
fn draw_playing(frame: &mut Frame, area: Rect, state: &MidiState, colors: &ThemeColors) {
    let modal_area = centered_modal_area(area);
    let modal = ModalFrame::themed(modal_area, " MIDI - Playing ", colors);
    modal.render_frame(frame);
    let content_area = modal.content_area();
    ...
    modal.render_help(frame, vec![("Esc", "stop & close")]);
}
